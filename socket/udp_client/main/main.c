...
#define EXAMPLE_UDP_SERVER_IP           "192.168.0.102"
#define EXAMPLE_UDP_SERVER_PORT         60001
#define EXAMPLE_UDP_LOCAL_PORT          60002
...
static void udp_client_task(void *pvParameters)
{
    ...
    local_addr.sin_port = htons(EXAMPLE_UDP_LOCAL_PORT);
    ...
    server_addr.sin_addr.s_addr = inet_addr(EXAMPLE_UDP_SERVER_IP);
    server_addr.sin_port = htons(EXAMPLE_UDP_SERVER_PORT);
    while (1) {
        tx_len = sendto(sock, "hello world 123", strlen("hello world 123"), 0, (struct sockaddr *)&server_addr, sizeof(server_addr));
        if (tx_len < 0) {
            ESP_LOGE(TAG, "socket send failed:%d", errno);
            break;
        }

        rx_len = recvfrom(sock, rx_buf, sizeof(rx_buf), 0, (struct sockaddr *)&remote_addr, &remote_addr_len);
        if (rx_len < 0) {
            ESP_LOGE(TAG, "socket recv failed:%d", errno);
            break;
        } else {
            rx_buf[rx_len] = 0;
            ESP_LOGI(TAG, "recv %s:%d %s", inet_ntoa(remote_addr.sin_addr), remote_addr.sin_port, rx_buf);
        }
    }
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (IP_EVENT == event_base) {
        ...
        case IP_EVENT_STA_GOT_IP:
            ...
            xTaskCreate(udp_client_task, "tcp_client", 4096, NULL, 5, NULL);
            break;
