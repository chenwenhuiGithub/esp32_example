...
#define EXAMPLE_UDP_SERVER_IP       "192.168.0.106"
...
static void udp_client_task(void *pvParameters)
{
    ...
    ser_addr.sin_addr.s_addr = inet_addr(EXAMPLE_UDP_SERVER_IP);
    ser_addr.sin_port = htons(EXAMPLE_UDP_SERVER_PORT);
    while (1) {
        tx_len = sendto(sock, "hello world 123", strlen("hello world 123"), 0, (struct sockaddr *)&ser_addr, sizeof(ser_addr));
        if (tx_len < 0) {
        ...
        rx_len = recvfrom(sock, rx_buf, sizeof(rx_buf) - 1, 0, (struct sockaddr *)&remote_addr, &remote_addr_len);
        if (rx_len < 0) {
            ESP_LOGE(TAG, "recv failed:%d", errno);
            break;
        } else {
            rx_buf[rx_len] = 0;
            ESP_LOGI(TAG, "recv from %s:%d %s", inet_ntoa(remote_addr.sin_addr), remote_addr.sin_port, rx_buf);
        }
    }
}
