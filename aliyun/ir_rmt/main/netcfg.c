...
static void led_init() {
    ...
    led_conf.pin_bit_mask = 1ULL << CONFIG_NETCFG_LED_GPIO_NUM;
    led_conf.mode = GPIO_MODE_OUTPUT;
    led_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
    led_conf.pull_up_en = GPIO_PULLUP_ENABLE;
    gpio_config(&led_conf);

    gpio_set_level(CONFIG_NETCFG_LED_GPIO_NUM, 0); // 0 - off, 1 - on
}

static void show_netstat_task(void* parameter) {	
    while (1) {
        if (NETSTAT_WIFI_NOT_CONNECTED == s_netstat) {
            gpio_set_level(CONFIG_NETCFG_LED_GPIO_NUM, 1);
            vTaskDelay(500 / portTICK_PERIOD_MS);
            gpio_set_level(CONFIG_NETCFG_LED_GPIO_NUM, 0);
            vTaskDelay(500 / portTICK_PERIOD_MS);
