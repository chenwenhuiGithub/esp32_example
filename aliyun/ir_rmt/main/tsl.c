...
void tsl_recv_set(uint8_t *payload, uint32_t len) {
    ...
    // tsl_send_set_reply(id_json->valuestring);
...
void tsl_send_set_reply(char *msg_id) {
    ...
    sprintf(buf, "{\"code\":200, \"data\":{}, \"id\":\"%s\", \"message\":\"success\", \"version\":\"1.0.0\"}", msg_id);
    cloud_send_publish(CONFIG_TOPIC_TSL_SET_REPLY, (uint8_t *)buf, strlen(buf));
}
...
void tsl_recv_post_reply(uint8_t *payload, uint32_t len) {
    ESP_LOGI(TAG, "post reply:%s", (char *)payload);
}