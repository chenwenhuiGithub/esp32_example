...
static size_t rmt_encode_ir_nec(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    ir_nec_frame_t *frame_data = (ir_nec_frame_t *)primary_data;
    ...
    // fall-through
    case 1: // send address
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &frame_data->address, sizeof(uint16_t), &session_state);
        if (session_state & RMT_ENCODING_COMPLETE) {
    ...
    // fall-through
    case 2: // send command
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &frame_data->command, sizeof(uint16_t), &session_state);
        if (session_state & RMT_ENCODING_COMPLETE) {
...
esp_err_t rmt_new_ir_nec_encoder(uint32_t resolution, rmt_encoder_handle_t *ret_encoder)
{
    ...
    nec_encoder->nec_leading_symbol = (rmt_symbol_word_t) {
        .level0 = 1,
        .duration0 = 9000ULL * resolution / 1000000,
        .level1 = 0,
        .duration1 = 4500ULL * resolution / 1000000,
    };
    ...
    rmt_bytes_encoder_config_t bytes_encoder_config = {
        .bit0 = {
            .level0 = 1,
            .duration0 = 560 * resolution / 1000000, // T0H=560us
            .level1 = 0,
            .duration1 = 560 * resolution / 1000000, // T0L=560us
        },
        .bit1 = {
            .level0 = 1,
            .duration0 = 560 * resolution / 1000000,  // T1H=560us
            .level1 = 0,
            .duration1 = 1690 * resolution / 1000000, // T1L=1690us
        },
    };
